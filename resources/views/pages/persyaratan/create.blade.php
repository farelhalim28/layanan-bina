...
<section class="section">
    <div class="card">
        <div class="card-header"><h4>Form Tambah Berkas Persyaratan</h4></div>
        <div class="card-body">
            <form action="{{ route('admin.berkas-persyaratan.store') }}" method="POST">
                @csrf
                <div class="row">
                    ...
                    <div class="col-md-12 mb-3">
                        ...
                        <select name="valid" id="valid" class="form-select @error('valid') is-invalid @enderror" required>
                            <option value="0" {{ old('valid') == '0' ? 'selected' : '' }}>Tidak Valid</option>
                            <option value="1" {{ old('valid') == '1' ? 'selected' : '' }}>Valid</option>
                        </select>
                        @error('valid')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
